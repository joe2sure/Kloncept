...
// import '../provider/dummy_provider.dart'; // Import the dummy provider
// import '../model/dummy/dummy_model.dart'; // Import dummy models
...
class _CoursesScreenState extends State<CoursesScreen> {
  ...
  // Convert DummyCourse to Course model if needed
  List<Course> _convertDummyCoursesToCourses(List<DummyCourse> dummyCourses) {
    return dummyCourses.map((dummy) => Course(
      id: dummy.id,
      userId: dummy.userId?.toString(),
      categoryId: dummy.categoryId?.toString(),
      title: dummy.title,
      shortDetail: dummy.shortDetail,
      detail: dummy.detail,
      price: dummy.price,
      discountPrice: dummy.discountPrice,
      previewImage: dummy.previewImage ?? dummy.imageUrl,
      type: dummy.type ?? "1", // Default to paid course
      createdAt: DateTime.now().subtract(Duration(days: dummy.id ?? 0)), // Just for demo
      // Add other required fields with dummy data
      whatlearns: [], // Add dummy whatlearns if needed
      include: [], // Add dummy includes if needed
    )).toList();
  }

  @override
  Widget build(BuildContext context) {
    final dummyCoursesProvider = Provider.of<DummyCoursesProvider>(context);
    final allDummyCourses = dummyCoursesProvider.allDummyCourses;
    final studyingCourses = allDummyCourses.where((course) => course.isEnrolled ?? false).toList();
    final wishlistCourses = allDummyCourses.where((course) => course.isFeatured ?? false).toList(); // Just for demo

    // Convert to Course model if needed (or modify Screen widget to accept DummyCourse)
    final allCourses = _convertDummyCoursesToCourses(allDummyCourses);
    final studying = _convertDummyCoursesToCourses(studyingCourses);
    final wishcourses = _convertDummyCoursesToCourses(wishlistCourses);
                ...
                Expanded(
                  ...
                  child: PageView(
                    ...
                    children: [
                      ...
                      Screen(studying, typecoursefil, _selectedIndex),
